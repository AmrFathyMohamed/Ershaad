...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $date = $_POST['date'];
    if ($date !== null) {
        $conditions[] = "appointments.Date >= '$date'";
    }
    ...
    if ($speciality != 'All') {
        $conditions[] = "Specialization = '$speciality'";
    }

    $priceRange = $_POST['priceRange'];
    if ($priceRange !== null) {
        $conditions[] = "PriceAfterPercentage > $priceRange";
    }
    ...
    if (!empty($conditions)) {
        ...
        $sql = "SELECT therapists.*, GROUP_CONCAT(appointments.Date) AS appointment_dates
                FROM therapists
                INNER JOIN appointments ON therapists.TherapistID = appointments.TherapistID AND appointments.Type = 'Work'
                WHERE (
                    (
                        (SELECT COUNT(*) FROM sessions WHERE sessions.TherapistID = therapists.TherapistID) = 0
                    )";
